...
# Exporter les poids de chaque couche dans des fichiers texte séparés
for i, layer in enumerate(model.layers):
    ...
    if weights_biases:
        ...
        print(f"Poids de la couche {layer.name} exportés avec succès dans layer_{i+1}_weights.txt")
        print(f"Biais de la couche {layer.name} exportés avec succès dans layer_{i+1}_biases.txt")
    else:
        print(f"Aucun poids ou biais disponibles pour la couche {layer.name}")
